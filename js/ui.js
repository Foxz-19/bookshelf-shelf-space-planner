...
export function renderEntries(entries, filter) {
  ...
  return visible.map((entry) => `<article class="entry" data-id="${entry.id}"><div class="entry-top"><span class="badge badge-${entry.status}">${STATUS_LABELS[entry.status]}</span><button class="icon-button delete" data-action="delete" aria-label="Remove ${escapeHtml(entry.name)}">×</button></div><h3>${escapeHtml(entry.name)}</h3><p class="method">${escapeHtml(entry.method)}</p><dl><div><dt>Started</dt><dd>${formatDate(entry.startedAt)}</dd></div></dl>${entry.note ? `<p class="note">${escapeHtml(entry.note)}</p>` : ''}<label class="status-control"><span>Progress</span><select data-action="status" aria-label="Update status for ${escapeHtml(entry.name)}">${Object.entries(STATUS_LABELS).map(([key, label]) => `<option value="${key}" ${key === entry.status ? 'selected' : ''}>${label}</option>`).join('')}</select></label></article>`).join('');
}
