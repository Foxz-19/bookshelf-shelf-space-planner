...
/** @type {import('./types').Trip} */ const defaults = { distance:480, efficiency:28, tank:14, price:3.59, start:75, unit:'imperial', currency:'USD' };
/** @type {Record<string,Pick<import('./types').Trip,'efficiency'|'tank'>>} */ const presets = { compact:{efficiency:32,tank:12}, suv:{efficiency:22,tank:18}, hybrid:{efficiency:50,tank:11} };
const ui=createUI(document); const loaded=loadTrip(); /** @type {import('./types').Trip} */ let state={...defaults,...(loaded.trip||{})}; let lastSaveError=null; ui.setInputs(state); if(loaded.error){ui.setSaveMessage(loaded.error);ui.toast(loaded.error,true);}
...
document.getElementById('trip-form').addEventListener('input',event=>{if(['efficiency','tank'].includes(/** @type {HTMLElement} */(event.target).id))/** @type {HTMLSelectElement} */(document.getElementById('vehicle')).value='custom';update();});
...
document.querySelectorAll('[data-unit]').forEach(button=>button.addEventListener('click',()=>{convertUnits(/** @type {'imperial'|'metric'} */(/** @type {HTMLButtonElement} */(button).dataset.unit));update();}));
