...
export function renderPlan(target: HTMLElement, plan: Plan, format: TimeFormat): void {
  ...
  target.innerHTML = `<div class="result-title"><p class="eyebrow">Your window</p><h2 id="results-title">${readableMinutes(plan.available)} to rest</h2><p class="best-note">Best fit: <strong>${best?.kind}</strong> matches this window.</p></div><div class="nap-list">${options}</div>`;
}
