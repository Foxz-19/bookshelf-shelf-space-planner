...
export function renderPlan(target: HTMLElement, plan: Plan, format: TimeFormat): void {
  ...
  const options = plan.options.map(option => `<article class="nap ${option.featured ? 'featured' : ''}"><div><p class="nap-kind">${option.kind}${option.featured ? '<span>best fit</span>' : ''}</p><h3>${readableMinutes(option.minutes)}</h3><p>${option.description}</p></div><div class="alarm"><small>SET ALARM FOR</small><strong>${readableTime(option.alarm, format)}</strong></div></article>`).join('');
  target.innerHTML = `<div class="result-title"><p class="eyebrow">Your window</p><h2 id="results-title">${readableMinutes(plan.available)} to rest</h2></div><div class="nap-list">${options}</div>`;
}
