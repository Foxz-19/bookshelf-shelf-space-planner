...
export function renderSummary(state: SleepState): void {
  ...
  const ring = byId<HTMLElement>('ring'); ring.style.setProperty('--amount', `${data.progress * 3.6}deg`); ring.setAttribute('aria-label', `${percent} percent of weekly sleep goal reached`); ring.title = `${percent}% of your weekly sleep goal reached`;
}
